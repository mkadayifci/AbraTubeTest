...
while True:
    ...
    humidity, temperature2 = Adafruit_DHT.read_retry(humSensor, pin)
    
    temperature  = sensor.readTemp()
    timeStr = str(datetime.datetime.now())
    temperatureString   = timeStr + ";" + str(temperature) +  "\r\n"
    os.system("clear")
    print('Temp={0:0.1f}*C  Humidity={1:0.1f}%'.format(temperature2, humidity))
    print 'Temperature = %3.3f C' % temperature
    with open('log','a') as f:
        f.write (str(datetime.datetime.now()) + ";" + str(temperature) + ";" + str(temperature2) + ";" + str(humidity) + ";" + str(cTemp) + ";" + str(pressure) +  "\r\n")

    
    #DateTime TSYSTemp DHT22Temp DHT22Hum MS5837Temp MS5837Pressure
